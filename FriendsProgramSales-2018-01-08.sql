//
// Friends Program Sales
// Catherine Warren, 2017-04-03 | JIRA RPT-611
// Edited Catherine Warren, 2017-11-08 | JIRA RPTG-27
// Edited Catherine Warren, 2018-01-02 & 01-05 | PivotalTracker #153791874
//
...
// The minimum auth subquery finds the minimum paymentauthorization_id for qualifying orders placed after Nov. 1, 2017.

var minAuthProcessor = new SelectSQLBuilder();

minAuthProcessor.setSelect("select first_value(authorization_id) OVER (PARTITION BY order_id) as authorization_id ");
minAuthProcessor.setFrom("from ecommerce.paymentauthorization ");
minAuthProcessor.setWhere("where payment_status_id IN (3, 5, 6) and payment_transaction_result_id = 1 and authdate >= '2017-11-01' ");
...
sqlProcessor.appendSelect("li.quantity, li.customerprice as customer_price, COALESCE(li.quantity,0) * COALESCE(li.customerPrice,0.00) AS lineitem_total ");
...
sqlProcessor.addCommonTableExpression("minAuths", minAuthProcessor);
sqlProcessor.setFrom("from ecommerce.paymentauthorization as pa RIGHT JOIN friendsOrds USING (order_id) RIGHT JOIN minAuths USING (authorization_id) INNER JOIN ecommerce.rslineitem as li USING (order_id) INNER JOIN ecommerce.productversion as pv USING (productversion_id) INNER JOIN ecommerce.item as i USING (item_id)");
...
sqlProcessor.appendWhere("s.subscription_id = spa.subscription_id and spa.authorization_id = minAuths.authorization_id");
