...
// QUERY 35
var versWeightedAvPrc = new SelectSQLBuilder();

versWeightedAvPrc.setSelect("select rsli.productVersion_id as versionId, sum(rsli.customerPrice/rsli.quantity) / sum(rsli.quantity) as price ");
versWeightedAvPrc.setFrom("from ecommerce.RSLineItem as rsli ");
versWeightedAvPrc.setWhere("where rsli.fulfillmentDate IS NOT NULL and rsli.fulfillmentDate >= '2015-01-01' and rsli.quantity > 0 ");
versWeightedAvPrc.setGroupBy("group by rsli.productVersion_id HAVING sum(rsli.quantity) > 0 ");


// QUERY 36
var Singles1 = new SelectSQLBuilder();

Singles1.setSelect("select pvsku.productVersion_id ");
Singles1.setFrom("from ecommerce.ProductVersionSKU as pvsku ");
Singles1.setWhere("where true ");
Singles1.setGroupBy("group by productVersion_id HAVING count(*) = 1 ");
...
SingleVersionPrice.addCommonTableExpression("vwap", versWeightedAvPrc);
SingleVersionPrice.addCommonTableExpression("s1", Singles1);
SingleVersionPrice.setFrom("from ecommerce.productversion as pv LEFT OUTER JOIN versWeightedAvPrc as vwap ON vwap.versionId = pv.productversion_id LEFT OUTER JOIN Singles1 as s1 ON pv.productversion_id = s1.productversion_id ");
SingleVersionPrice.setWhere("where true ");


// QUERY 37.5 Singles2
var Singles2 = new SelectSQLBuilder();

Singles2.setSelect("select pvsku.productVersion_id ");
Singles2.setFrom("from ecommerce.ProductVersionSKU as pvsku ");
Singles2.setWhere("where true ");
Singles2.setGroupBy("group by productVersion_id HAVING count(*) = 1 ");
...
TMP_SKU_PRICE.addCommonTableExpression("s2", Singles2);
TMP_SKU_PRICE.setFrom("from ecommerce.price as pr, ecommerce.ProductVersionSKU as pvsku LEFT OUTER JOIN SingleVersionPrice as svp ON pvsku.productversion_id = svp.versionId LEFT OUTER JOIN Singles2 as s2 ON pvsku.productVersion_id = s2.productVersion_id ");
...
mSKUPRICE.setSelect("select tmp_sku_price.skuId, min(tmp_sku_price.customerprice) as price ");
mSKUPRICE.addCommonTableExpression("tmp_sku_price", TMP_SKU_PRICE);
mSKUPRICE.setFrom("from ecommerce.sku as s RIGHT JOIN TMP_SKU_PRICE as tmp_sku_price ON s.sku_id = tmp_sku_price.skuId ");
mSKUPRICE.setWhere("where tmp_sku_price.customerprice > 0 ");
mSKUPRICE.setGroupBy("group by tmp_sku_price.skuId ");
...
// QUERY 40.5 
var mSKUPRICE2 = new SelectSQLBuilder();

mSKUPRICE2.setSelect("select msp.skuId, msp.price ");
mSKUPRICE2.addCommonTableExpression("msp", mSKUPRICE);
mSKUPRICE2.setFrom("from ecommerce.sku as s RIGHT JOIN mSKUPRICE as msp ON s.sku_id = msp.skuId ");
mSKUPRICE2.setWhere("where true ");
mSKUPRICE2.setGroupBy("group by msp.skuId ");
...
versionSkuRevPercentage.appendSelect("avg(CASE WHEN vp.price > 0 THEN (msp2.price * pvsku.quantity) / vp.price ELSE 0 END) as percentage ");
versionSkuRevPercentage.addCommonTableExpression("vp", VERSION_PRICE);
versionSkuRevPercentage.addCommonTableExpression("msp2", mSKUPRICE2);
versionSkuRevPercentage.setFrom("from ecommerce.ProductVersionSKU as pvsku INNER JOIN VERSION_PRICE as vp ON pvsku.productVersion_id = vp.versionId LEFT OUTER JOIN mSKUPRICE2 as msp2 ON pvsku.sku_id = msp2.skuId ");
