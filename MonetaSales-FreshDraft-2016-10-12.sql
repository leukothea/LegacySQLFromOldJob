...
order by extract( 'year' from rsli.fulfillmentDate) DESC 
 ...
 , vac AS (  WITH tempnext AS (select ii.sku_id, (CASE WHEN (ii.quantity = 0 OR ii.merchantprice = 0) THEN max(ii.merchantPrice) WHEN (ii.quantity > 0 AND ii.merchantPrice > 0) THEN sum(ii.quantity * ii.merchantPrice) / sum(ii.quantity) ELSE NULL END) as cost 
from ecommerce.RSInventoryItem as ii 
where ii.quantity > 0 and ii.merchantPrice > 0 and ii.active = true and ii.sku_id is not null 
group by ii.sku_id, ii.quantity, ii.merchantprice 
 )
...
ORDER BY maxSkuDates.skuId 
 ...
 select skudetails.skuFamilyId, skudetails.skuFamilyName, skudetails.skuFamilyStatusId, skudetails.skuFamilyStatus, skudetails.skuFamilyVendor, skudetails.skuId, skudetails.skuStatus, skudetails.tracksInventory ,skudetails.skuName, skudetails.partnumber, skudetails.countryCode, skudetails.skuBuyer, skudetails.skucategory1, skudetails.skucategory2, skudetails.skucategory3, skudetails.skucategory4, skudetails.skucategory5, skudetails.skucategory6 ,skudata.skuQuantity, skudata.skuLowerOfCost, skudata.skuWeight, skudata.skuSuppliers, skucalc.skuReorderDate, skucalc.skuReorderAge, skucalc.skuPrice, skucalc.skuAverageCost ,skucalc.skuInitialCost, skucalc.skuCurrentCost, mostrecentsupplier.supplierName 
...
where true  and skuMain.skuId = 65334
