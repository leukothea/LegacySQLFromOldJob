...
// Edited Catherine Warren, 2015-12-14 - 16 | JIRA RPT-197
// Made into the report of record, 2016-01-05
// Edited Catherine Warren, 2016-01-19 | JIRA RPT-220
//
...
var osshoppingcart = p["OrderSource-ShoppingCart"];
var oscustserv = p["OrderSource-CustomerService"];
var osamazcoupaw = p["OrderSource-Amazon-Coupaw"];
var osamazdoggyloot = p["OrderSource-Amazon-Doggyloot"];
var osgreatergood = p["OrderSource-Amazon-GreaterGood"];
var osggr = p["OrderSource-Amazon-GGR"];
...
// BLOCK OF CODE TO HANDLE NEW CHANNEL CHECKBOXES
var ids = "";

if (notEmpty(osshoppingcart)) {
    if(ids.length > 0) {ids += ",";
                               }
    ids += "1";
}

if (notEmpty(oscustserv)) {
    if(ids.length > 0) {ids += ",";
                               }
    ids += "2";
}

if (notEmpty(osamazcoupaw)) {
    if(ids.length > 0) {ids += ",";
                               }
    ids += "5";
}

if (notEmpty(osamazdoggyloot)) {
    if(ids.length > 0) {ids += ",";
                               }
    ids += "6";
}

if (notEmpty(osgreatergood)) {
    if(ids.length > 0) {ids += ",";
                               }
    ids += "3";
} 
...
// END BLOCK FOR CHANNEL CHECKBOXES
...
if (ids.length == 0) {
    ...
    stuffProcessor.appendWhere("o.order_source_id IN (" + ids + ")");
} 
...
if (ids.length == 0) {
    ...
    royaltySubProcessor.appendWhere("o.order_source_id IN (" + ids + ")");
} 
...
if (ids.length == 0) {
    ...
    royaltyProcessor.appendWhere("o.order_source_id IN (" + ids + ")");
} 
...
if (ids.length == 0) {
    ...
    sqlProcessor.appendWhere("o.order_source_id = os.order_source_id and o.order_source_id IN (" + ids + ")");
