...
sqlProcessor.setFrom("from ecommerce.PurchaseOrder as po, ecommerce.Supplier as s ");
sqlProcessor.appendRelationToFromWithAlias(tmpPoQuantitySqlProcessor, "tpoq");
sqlProcessor.setWhere("where tpoq.po_id = po.purchaseorder_id ");
...
if (notEmpty(show_all_line_items)) {
    ...
    sqlProcessor.appendFrom("ecommerce.SKU as pv, ecommerce.ReceivingStatus as rs, ecommerce.supplier_address as a ");
    ...
    sqlProcessor.appendWhere("po.supplier_id = s.supplier_id and s.supplier_id = a.supplier_id and a.is_primary = true ");
} else {
    sqlProcessor.appendFrom("ecommerce.purchaseorderlineitem as poli ");
    sqlProcessor.appendWhere("po.purchaseOrder_id = poli.purchaseOrder_id ");
}
