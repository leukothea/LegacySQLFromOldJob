Current version as of 2016-06-14
...
// took 2442 ms
 ...
 , vsrp AS 
      (  WITH versionPrice AS 
            (  WITH minskuprice AS 
                  (  WITH tmp_sku_price AS 
                        (  WITH svp AS 
                              (  WITH vwap AS 
...
// took 1182 ms
 ...
 select 
      vwap.versionId as versionId, vwap.price 
from ecommerce.productversion as pv LEFT OUTER JOIN vwap as vwap ON vwap.versionId = pv.productversion_id RIGHT JOIN singles as singles ON pv.productversion_id = singles.productversion_id 
where true 
 )
 select 
      (CASE WHEN (pr.pricetype_id = 1 AND pr.sourceclass_id = 13) THEN pr.source_id ELSE pvsku.sku_id END) as skuId ,(CASE WHEN (pr.pricetype_id = 1 AND pr.sourceclass_id = 13) THEN pr.customerprice ELSE avg(svp.price / pvsku.quantity) END) as customerprice 
from ecommerce.price as pr, ecommerce.ProductVersionSKU as pvsku LEFT OUTER JOIN svp as svp ON pvsku.productversion_id = svp.versionId 
where pvsku.quantity > 0 
group by skuId, pr.pricetype_id, pr.sourceclass_id, pr.customerprice 
 )
 select 
      tmp_sku_price.skuId, min(tmp_sku_price.customerprice) as price 
from ecommerce.sku as s RIGHT JOIN TMP_SKU_PRICE as tmp_sku_price ON s.sku_id = tmp_sku_price.skuId 
where tmp_sku_price.customerprice > 0 
group by tmp_sku_price.skuId 
 )
 select 
      pvsku.productVersion_id as versionId, sum(coalesce(minskuprice.price,0) * pvsku.quantity ) as price 
from ecommerce.productversionsku as pvsku INNER JOIN minskuprice as minskuprice ON pvsku.sku_id = minskuprice.skuId 
where true 
group by pvsku.productversion_id 
 )
 select pvsku.productVersion_id as versionId, pvsku.sku_id as skuId ,avg(CASE WHEN versionPrice.price > 0 THEN (versionPrice.price * pvsku.quantity) / versionPrice.price ELSE 0 END) as percentage 
from ecommerce.ProductVersionSKU as pvsku INNER JOIN versionPrice as versionPrice ON pvsku.productVersion_id = versionPrice.versionId 
where true
group by pvsku.productVersion_id, pvsku.sku_id 
 )

// the above nested queries never resolved - so I think this is the issue! 
...
where v.itemStatus_id = ist.itemStatus_id and v.item_id = i.item_id 
 ...
 , versionsales AS (select rsli.productVersion_id::varchar as versionId, extract( 'year' from rsli.fulfillmentDate) as year, sum(rsli.quantity) as units, sum(rsli.customerPrice * rsli.quantity) as revenue 
...
group by rsli.productVersion_id::varchar, extract( 'year' from rsli.fulfillmentDate) 
...
order by versionDetails.versionId asc
 ...
 , skuMain AS (  WITH skucalc AS (select s.sku_id ,COALESCE(max(rcvd.receiveddate::DATE), s.initialLaunchDate::DATE, s.daterecordadded::DATE) AS skuReorderDate ,(abs(EXTRACT(DAY FROM COALESCE(rcvd.receiveddate, ii.daterecordadded, s.daterecordadded, s.initiallaunchdate) - now())))/365 AS skuReorderAge , 0 AS skuPrice , 0 AS skuAverageCost , 1 AS skuSoldAsSingles , 0 AS skuInitialCost , 0 AS skuCurrentCost 
from ecommerce.sku as s, ecommerce.rsinventoryitem as ii, ecommerce.receivingevent as rcvd 
where ii.sku_id = s.sku_id and ii.receivingevent_id = rcvd.receivingevent_id 
group by s.sku_id, rcvd.receiveddate, s.initialLaunchDate, s.daterecordadded, ii.daterecordadded 
 )
...
where sku.itemStatus_id = ist.itemStatus_id AND sku.sku_class_id = sc.sku_class_id AND sku.item_id = i.item_id  and i.itemStatus_id = istB.itemStatus_id AND v.vendor_id = i.vendor_id 
...
where vsid.sku_id = vsrp.skuId  and skuMain.skuCategory1 ILIKE '%Apparel%'  and skuMain.skuCategory2 ILIKE '%Purple Paw or Animal Icon%' 
