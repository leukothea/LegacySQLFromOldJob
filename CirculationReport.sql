...
var dateClause = "";
if (notEmpty(days)) {
	dateClause = dateClause + "s.date_record_added >= now()::DATE - cast('" + days + " day' as interval)";
    if (days == 0) {
         dateClause = dateClause + " and s.date_record_added > now()::DATE";
    } else {
         dateClause = dateClause + " and s.date_record_added < now()::DATE";
    }
} else {
    ...
    if (notEmpty(startDate)) {
    	dateClause = dateClause + "s.date_record_added >= '" + startDate + "'";
        if (notEmpty(endDate)) {
        	dateClause = dateClause + " and s.date_record_added < '" + endDate + "'";
        }
    } else {
    	dateClause = dateClause + "s.date_record_added >= date_trunc('month',now()::DATE)";
    }
}
...
dateProcessor.appendSelect("count(*) AS subscribed_count, s.date_record_added::DATE as date_confirmed ");
...
dateProcessor.setGroupBy("group by so.description, s.date_record_added::DATE ");
