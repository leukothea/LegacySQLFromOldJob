...
// Taken from Promotion Code Report. Edited by Catherine Warren, 2015-11-18, to include promotions lack a code (RPT-184)
...
// Edited Catherine Warren, 2016-01-19, JIRA RPT-217
//
...
if("ignoreAll" == siteId) {
    hide.push("site_name");
} else if("showAll" == siteId) { //break out by site
    sqlProcessor.appendSelect("s.name as site_name ");
    sqlProcessor.appendFrom("ecommerce.rsorder as o, ecommerce.site as s ");
    sqlProcessor.appendWhere("o.oid = pa.order_id and o.oid = op.order_id and o.site_id = s.site_id ");
