with wrapper as (
	with orderlevel as(
		WITH order_item_types AS (
			SELECT
				o.oid as order_id
				,SUM(CASE WHEN (i.itembitmask & 32 = 32 AND rsli.lineitemtype_id = 1 AND (sli.sourceclass_id != 22 OR sli.sourceclass_id IS NULL)) THEN 1 ELSE 0 END) as realGTGM
				,SUM(CASE WHEN (i.itembitmask & 32 = 32 AND rsli.lineitemtype_id = 1 AND sli.sourceclass_id = 22 ) THEN 1 ELSE 0 END) as checkboxGTGM
		...
		oit.order_id
		, CASE WHEN (oit.realGTGM > 0 OR oit.checkboxGTGM > 0) AND oit.nonGTGM = 0 THEN 'gtgm only'
			WHEN oit.realGTGM > 0 AND oit.checkboxGTGM = 0 AND oit.nonGTGM > 0 THEN 'mixed_product_and_realGTGMonly'
			WHEN oit.realGTGM = 0 AND oit.checkboxGTGM > 0 AND oit.nonGTGM > 0 THEN 'mixed_product_and_checkboxGTGMonly'
			WHEN oit.realGTGM > 0 AND oit.checkboxGTGM > 0 AND oit.nonGTGM > 0 THEN 'mixed_product_real_and_checkboxGTGM'
			WHEN (oit.realGTGM = 0 AND oit.checkboxGTGM = 0) AND oit.nonGTGM > 0 THEN 'product only'
