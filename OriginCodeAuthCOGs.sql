...
currentSkuCostProcessor.setFrom("FROM ecommerce.SKU as s, ecommerce.ProductVersionSKU as pvs, ecommerce.RSInventoryItem as ii LEFT OUTER JOIN dra ON ii.sku_id = dra.sku_id ");
currentSkuCostProcessor.addCommonTableExpression("dra",maxIIDateProcessor);
//currentSkuCostProcessor.appendRelationToFromWithAlias(maxIIDateProcessor, "dra");
currentSkuCostProcessor.setWhere("WHERE s.sku_id = ii.sku_id and s.sku_id = pvs.sku_id and s.skuBitMask & 1 = 1 and dra.dra = ii.dateRecordAdded and ii.merchantPrice > 0 ");
...
currentVersionCostProcessor.setFrom("FROM ecommerce.productversionsku as pvsku LEFT OUTER JOIN csc ON pvsku.sku_id = csc.sku_id ");
currentVersionCostProcessor.addCommonTableExpression("csc",currentSkuCostProcessor);
...
currentOrderCostProcessor.setFrom("FROM ecommerce.paymentauthorization as pa, ecommerce.rslineitem as li LEFT OUTER JOIN cvc ON li.productversion_id = cvc.version_id ");
currentOrderCostProcessor.addCommonTableExpression("cvc",currentVersionCostProcessor);
...
currentOrderCostProcessor.setWhere("WHERE pa.order_id = li.order_id "); 
currentOrderCostProcessor.appendWhere("pa.payment_transaction_result_id = 1 and pa.payment_status_id in (3,5,6) ");
currentOrderCostProcessor.appendWhere(dateClause);
...
gtgmProcessor.setFrom("from ecommerce.rslineitem as li,ecommerce.productversion as pv,ecommerce.item as i, ecommerce.paymentauthorization as pa ");
gtgmProcessor.setWhere("where li.productversion_id = pv.productversion_id and pv.item_id = i.item_id and i.itembitmask & 32 = 32 and li.order_id = pa.order_id ");
...
sqlProcessor.appendFrom("ecommerce.RSOrder as o LEFT OUTER JOIN gtgm ON o.oid = gtgm.order_id LEFT OUTER JOIN cocp ON o.oid = cocp.order_id LEFT OUTER JOIN ry on o.oid = ry.order_id ");
sqlProcessor.addCommonTableExpression("cocp", currentOrderCostProcessor);
sqlProcessor.addCommonTableExpression("gtgm", gtgmProcessor);
sqlProcessor.addCommonTableExpression("ry", royaltyProcessor);
sqlProcessor.setWhere("where pa.order_id = o.oid and pa.order_id = ry.order_id and pa.order_id = cocp.order_id and gtgm.order_id = pa.order_id ");
