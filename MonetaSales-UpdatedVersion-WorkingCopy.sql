...
order by extract( 'year' from rsli.fulfillmentDate) DESC 
 ...
 , vac AS (  WITH tempnext AS (
	select ii.sku_id, (CASE WHEN ii.quantity = 0 OR ii.merchantprice = 0 THEN max(ii.merchantPrice) WHEN ii.quantity > 0 AND ii.merchantPrice > 0 THEN sum(ii.quantity * ii.merchantPrice) / sum(ii.quantity) ELSE NULL) as cost 
