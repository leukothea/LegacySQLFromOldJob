...
skudetails.appendSelect("skucat.skucategory1, skucat.skucategory2, skucat.skucategory3, skucat.skucategory4, skucat.skucategory5, skucat.skucategory6 ");
...
skudetails.appendFrom("ecommerce.Item as i, ecommerce.ItemStatus as istB, ecommerce.vendor as v ");
skudetails.setWhere("where sku.itemStatus_id = ist.itemStatus_id AND sku.item_id = i.item_id ");
skudetails.appendWhere("i.itemStatus_id = istB.itemStatus_id AND v.vendor_id = i.vendor_id ");
//skudetails.appendWhere("(sku.date_retired IS NULL OR sku.date_retired::DATE >= date_trunc('month',now()::DATE) - cast('5 years' as interval)) "); // this line suppresses old retired SKUs
// COMMENT OUT the above line to remove retired versions unless they have sold within the past 5 years.
...
skuMain.appendSelect("skudata.skuQuantity, skudata.skuLowerOfCost, skudata.skuWeight, skudata.skuSuppliers, skucalc.skuReorderDate, skucalc.skuReorderAge, skucalc.skuPrice, skucalc.skuAverageCost ");
...
tempnext.setSelect("select ii.sku_id, (CASE WHEN (ii.quantity = 0 OR ii.merchantprice = 0) THEN max(ii.merchantPrice) WHEN (ii.quantity > 0 AND ii.merchantPrice > 0) THEN sum(ii.quantity * ii.merchantPrice) / sum(ii.quantity) ELSE NULL END) as cost ");
tempnext.setFrom("from ecommerce.RSInventoryItem as ii ");
tempnext.setWhere("where ii.quantity > 0 and ii.merchantPrice > 0 and ii.active = true and ii.sku_id is not null ");
tempnext.setGroupBy("group by ii.sku_id, ii.quantity, ii.merchantprice ");
...
sqlProcessor.setSelect("select DISTINCT vsid.productversion_id as version_id, vsid.sku_id as sku_id, versionSkuQuantity.quantity AS versionSkuQuantity, '0' as skuRevenuePercentage ");
...
sqlProcessor.appendSelect("skuMain.skuQuantity, skuMain.skuLowerOfCost, skuMain.skuWeight, skuMain.skuSuppliers, skuMain.skuReorderDate, skuMain.skuReorderAge, skuMain.skuPrice, skuMain.skuAverageCost ");
