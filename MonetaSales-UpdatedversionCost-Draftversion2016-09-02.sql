...
4) Redo the vd.versionAverageCost column to be in line with the sql assembly version. 
...
order by extract( 'year' from rsli.fulfillmentDate) DESC 
 ...
 , vac AS (  WITH tempnext AS (
	select ii.sku_id, (CASE WHEN ii.quantity = 0 OR ii.merchantprice = 0 THEN max(merchantPrice) ELSE WHEN ii.quantity > 0 AND merchantPrice > 0 THEN sum(quantity * merchantPrice) / sum(quantity) ELSE NULL) as cost 
